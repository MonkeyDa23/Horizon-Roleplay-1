...
export const setupCoreModule = (client: Client) => {
    ...
    app.use(helmet());
    app.use(cors({ 
        origin: ['https://novaroleplay.com', 'http://localhost:3000', 'https://ais-pre-ybw2kepvyjl3nudev22cgi-28074720729.europe-west2.run.app'] 
    }));
    ...
    app.post('/sync-user/:discordId', authenticate, async (req, res) => {
        ...
        try {
            if (!env.DISCORD_GUILD_ID) {
                return res.status(500).json({ error: 'System not configured: DISCORD_GUILD_ID missing' });
            }
