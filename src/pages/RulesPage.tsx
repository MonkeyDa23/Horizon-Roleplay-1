...
const RulesPage: React.FC = () => {
  ...
  const renderContent = () => {
    ...
    if (error) {
      return (
        <div className="glass-panel p-10 text-center text-red-400">
          ...
          <h2 className="text-2xl font-bold mb-2">Failed to Load Rules</h2>
          <p>{error}</p>
        </div>
      );
    }
    ...
    return ruleCategories.map((category, index) => (
      <div key={category.id} className="glass-panel overflow-hidden animate-stagger hover:shadow-glow-blue-light" style={{ animationDelay: `${index * 100}ms` }}>
        <button
