...
import type { StaffMember, DiscordWidget } from '../types';
  ...
  return (
    ...
    >
      <div className="relative mb-6">
        ...
        />
        <div className="absolute inset-0 rounded-full border border-white/10" pointerEvents="none" />
      </div>
      <h3 className="text-2xl font-black text-white mb-2">{member.username}</h3>
      <span 
        className="px-4 py-2 bg-white/5 rounded-xl border border-white/5 font-bold text-xs uppercase tracking-widest" 
        style={{ color: branding.primaryColor }}
      >
        {t(member.role_key) || member.role_key}
      </span>
    </div>
  );
});
...
const AboutUsPage: React.FC = () => {
  ...
  const renderActiveView = () => {
      ...
      return (
        <div className="space-y-10">
          <div className="flex justify-between items-center mb-10">
            <h2 className="text-3xl md:text-5xl font-black text-white">{t('meet_the_team')}</h2>
          </div>
          {staff.length > 0 ? (
            ...
            <div className="text-center bg-white/5 border border-white/10 rounded-[40px] p-10">
              <p className="text-text-secondary text-lg font-bold">
                {t('staff_empty') || 'قائمة فريق العمل فارغة حالياً.'}
              </p>
            </div>
          )}
        </div>
    ...
    if (activeView === 'servers') {
      return (
        <div className="space-y-10">
          <h2 className="text-3xl md:text-5xl font-black text-center mb-10">{t('our_servers')}</h2>
  ...
  return (
    <>
      <SEO 
        title={`${communityName} - ${t('about_us') || 'من نحن'}`}
        description={`Learn more about the ${communityName} roleplay community.`}
      />

      <div className="container-custom">
        <div className="text-center max-w-4xl mx-auto space-y-8 animate-fade-in-up">
          <div 
            className="inline-flex p-6 rounded-[32px] shadow-2xl items-center justify-center border" 
            style={{ 
              backgroundColor: `${config.branding?.primaryColor || '#00A9FF'}22`, 
        ...
        <div className="bg-white/[0.02] border border-white/10 rounded-[48px] p-2 backdrop-blur-xl flex flex-wrap gap-2 max-w-xl mx-auto">
          <button 
            onClick={() => setActiveView('staff')}
            className={`flex-1 py-5 px-8 rounded-[40px] font-black text-base transition-all flex items-center justify-center gap-3 ${activeView === 'staff' ? 'bg-white text-brand-dark shadow-xl' : 'text-text-secondary hover:text-white hover:bg-white/5'}`}
          >
            <Users size={22} />
            {t('server_management')}
          </button>
          <button 
            ...
            className={`flex-1 py-5 px-8 rounded-[40px] font-black text-base transition-all flex items-center justify-center gap-3 ${activeView === 'servers' ? 'bg-white text-brand-dark shadow-xl' : 'text-text-secondary hover:text-white hover:bg-white/5'}`}
          >
            <Server size={22} />
            {t('our_servers')}
          </button>
