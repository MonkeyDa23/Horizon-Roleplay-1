...
const HealthCheckPage: React.FC = () => {
    ...
    const handleRunSyncTest = async () => {
        ...
        try {
            ...
            setSyncResult({ ok: false, error: (error as Error).message, status: (error as ApiError).status });
        } finally {
    ...
    const renderSyncResultInterpretation = () => {
        ...
        if (syncResult.status === 404) {
            ...
            interpretation = t('health_check_result_503');
        } else {
            interpretation = t('health_check_result_other');
        }

        return (
            <div className="mt-4 p-4 rounded-md bg-brand-dark">
                <h4 className="font-bold text-lg mb-2 text-yellow-300">{t('health_check_result_interpretation')}</h4>
                <div className="text-gray-300 text-sm" dangerouslySetInnerHTML={{ __html: interpretation }} />
            </div>
        )
    }
                ...
                <div className="max-w-4xl mx-auto">
                    ...
                    <div className="space-y-8">
                         <div className="bg-brand-dark-blue p-6 rounded-lg border-2 border-brand-light-blue shadow-lg">
                            ...
                            <div className="flex items-center justify-center gap-1 md:gap-2 flex-wrap p-4 bg-brand-dark rounded-lg text-center">
                                ...
                                <div className="flex flex-col items-center p-2">
                                    <DiscordLogo className="w-7 h-7 text-brand-cyan" />
                                    <span className="text-xs mt-1">Discord<br/>API</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="bg-brand-dark-blue p-6 rounded-lg border-2 border-brand-light-blue shadow-lg">
                            ...
                            {syncResult && (
                                <>
                                    <pre className={`mt-4 bg-brand-dark p-4 rounded-md text-sm whitespace-pre-wrap ${syncResult.ok ? 'text-green-300' : 'text-red-300'}`}>{JSON.stringify(syncResult, null, 2)}</pre>
