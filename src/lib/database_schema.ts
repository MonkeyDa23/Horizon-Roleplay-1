...
-- Remove obsolete manual permission flags
...
-- =============================================================================
--  1.5. ROLE-BASED ACCESS CONTROL (RBAC) SYSTEM
-- =============================================================================

-- Table to store permissions for each Discord role
...
CREATE POLICY "Allow admin read access" ON public.role_permissions
  FOR SELECT USING (public.has_permission('admin_permissions'));
-- Inserts/Updates are handled by a secure RPC function.
...
-- RPC for admins with 'admin_permissions' to update role permissions
...
BEGIN
  -- Only users with the 'admin_permissions' permission can run this.
  IF NOT public.has_permission('admin_permissions') THEN
  ...
  VALUES (
    ...
    'Updated permissions for role ' || p_role_id || '. Set permissions to: ' || array_to_string(p_permissions, ', ')
  );
END;
$$;


-- RPC for admins to log their access
...
DECLARE
  ...
  current_admin_username text;
BEGIN
  IF NOT public.has_permission('admin_panel') THEN
...
CREATE POLICY "Allow admin update access" ON public.config
  FOR UPDATE USING (public.has_permission('admin_appearance'));
...
CREATE POLICY "Allow admin full access" ON public.quizzes FOR ALL USING (public.has_permission('admin_quizzes'));
...
CREATE POLICY "Allow admin read access" ON public.submissions
  FOR SELECT USING (public.has_permission('admin_submissions'));
...
BEGIN
  IF NOT public.has_permission('admin_submissions') THEN
...
CREATE POLICY "Allow admin full access" ON public.products FOR ALL USING (public.has_permission('admin_store'));
...
CREATE POLICY "Allow admin full access" ON public.rule_categories FOR ALL USING (public.has_permission('admin_rules'));
...
CREATE POLICY "Allow admin full access" ON public.rules FOR ALL USING (public.has_permission('admin_rules'));
...
CREATE POLICY "Allow admin read access" ON public.audit_logs
  FOR SELECT USING (public.has_permission('admin_audit_log'));
...
CREATE POLICY "Allow admin full access" ON public.translations FOR ALL USING (public.has_permission('admin_translations'));

-- Function for admins to bulk-update translations
...
BEGIN
  ...
  LOOP
    ...
    SET en = EXCLUDED.en, ar = EXCLUDED.ar, updated_at = EXCLUDED.updated_at;
  END LOOP;
...
BEGIN
  IF TG_OP = 'INSERT' THEN
    ...
    payload := jsonb_build_object( 'type', event_type, 'payload', jsonb_build_object( 'userId', new.user_id, 'username', new.username, 'quizTitle', new."quizTitle" ) );
  ELSIF TG_OP = 'UPDATE' AND new.status <> old.status THEN
    ...
    ELSE RETURN new;
    END IF;
    payload := jsonb_build_object( 'type', event_type, 'payload', jsonb_build_object( 'userId', new.user_id, 'username', new.username, 'quizTitle', new."quizTitle", 'status', new.status, 'adminUsername', new."adminUsername" ) );
  ELSE RETURN new;
  END IF;
