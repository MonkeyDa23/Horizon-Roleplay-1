...
export const translations: Translations = {
  ...
  // New visual helper for API key error
  login_error_api_key_title: { en: 'API Key Mismatch Detected', ar: 'تم اكتشاف عدم تطابق في مفتاح API' },
  login_error_api_key_intro: { en: 'The secret key used by the website is different from the one the bot is expecting. These two keys **must be identical** for the connection to work.', ar: 'المفتاح السري الذي يستخدمه الموقع يختلف عن المفتاح الذي يتوقعه البوت. **يجب أن يكون هذان المفتاحان متطابقين تماماً** حتى يعمل الاتصال.' },
  login_error_website_env: { en: 'Website .env File', ar: 'ملف .env الخاص بالموقع' },
  login_error_bot_env: { en: 'Bot .env File', ar: 'ملف .env الخاص بالبوت' },
  login_error_must_match: { en: 'Must Match', ar: 'يجب أن يتطابق' },
};