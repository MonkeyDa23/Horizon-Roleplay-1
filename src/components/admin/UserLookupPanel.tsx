            ...
            <Modal isOpen={isBanModalOpen} onClose={() => setBanModalOpen(false)} title={t('confirm_ban')}>
                <div className="space-y-4">
                    <div>
                        ...
                        {/* FIX: Use e.currentTarget.value to correctly access the input's value. */}
                        <input type="text" value={banReason} onChange={e => setBanReason(e.currentTarget.value)} className="vixel-input" />
                    </div>
                    <div>
                        <label className="block font-semibold mb-1">{t('duration')} (in hours)</label>
                        {/* FIX: Use e.currentTarget.value to correctly access the input's value. */}
                        <input type="number" onChange={e => setBanDuration(parseInt(e.currentTarget.value) || null)} placeholder="Leave empty for permanent" className="vixel-input" />
                    </div>
