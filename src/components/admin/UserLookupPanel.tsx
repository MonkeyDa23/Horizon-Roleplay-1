            ...
            <Modal isOpen={isBanModalOpen} onClose={() => setBanModalOpen(false)} title={t('confirm_ban')}>
                <div className="space-y-4">
                    <div>
                        ...
                        <input type="text" value={banReason} onChange={e => setBanReason(e.target.value)} className="w-full bg-brand-light-blue p-2 rounded border border-gray-600" />
                    </div>
                    <div>
                        <label className="block font-semibold mb-1">{t('duration')} (in hours)</label>
                        <input type="number" onChange={e => setBanDuration(parseInt(e.target.value) || null)} placeholder="Leave empty for permanent" className="w-full bg-brand-light-blue p-2 rounded border border-gray-600" />
                    </div>
