...
const NavLink: React.FC<{ to: string; children: React.ReactNode; onClick?: () => void }> = ({ to, children, onClick }) => {
    ...
    return (
        <RouterNavLink
            ...
            onClick={onClick}
            className={({ isActive }) =>
                `relative text-text-secondary hover:text-white transition-all duration-500 font-extrabold tracking-tight group py-2
                ${isActive ? 'text-white' : 'opacity-60'}`
            }
            end={to === '/'}
        >
            {children}
            <span className={`absolute -bottom-1 left-0 w-full h-1 rounded-full transition-all duration-500 origin-left 
                scale-x-0 group-hover:scale-x-50 py-[2px]
...
const UserMenuLink: React.FC<{ to: string; children: React.ReactNode; icon: React.ElementType; onClick?: () => void }> = ({ to, children, icon: Icon, onClick }) => {
    const { branding } = useConfig();
    return (
        <Link to={to} onClick={onClick} className="flex items-center gap-4 w-full text-start px-5 py-4 text-sm font-black text-text-secondary hover:bg-white/[0.05] hover:text-white rounded-[20px] transition-all group">
              ...
              <div className="flex items-center gap-4">
                ...
                {user ? (
                  <div className="relative">
                    ...
                    >
                       ...
                       <div className="text-start hidden sm:block">
                           <p className="text-[10px] font-black text-text-secondary uppercase tracking-widest opacity-40 leading-none mb-1">{t('welcome')}</p>
                           <p className="text-sm font-black text-white tracking-tight leading-none">{user.username}</p>
                       </div>
                    ...
                    {userDropdownOpen && (
                       <div className="absolute top-full mt-4 right-0 bg-brand-dark border border-white/10 p-3 w-64 rounded-[40px] animate-fade-in-up origin-top-right shadow-2xl">
                         ...
                         <button onClick={logout} className="flex items-center gap-4 w-full text-start px-5 py-4 text-sm font-black text-red-500 hover:bg-red-500/10 rounded-[20px] transition-all group">
                  ...
                  <button
                    ...
                    disabled={loading}
                    className="relative overflow-hidden group h-12 px-8 rounded-2xl flex items-center gap-3 font-black text-sm tracking-tight text-brand-dark transition-all hover:scale-105 active:scale-95 disabled:opacity-50"
                    style={{ backgroundColor: branding.primaryColor }}
                  >
                ...
                <div className="lg:hidden">
                     <button onClick={() => setMobileMenuOpen(true)} className="w-12 h-12 flex items-center justify-center rounded-2xl bg-white/5 hover:bg-white/10 transition-all text-text-secondary">
                         <Menu size={28} />
                     </button>
                </div>
              </div>
      ...
      <AnimatePresence>
          {mobileMenuOpen && (
              <motion.div 
                ...
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                className="fixed inset-0 z-[100] bg-brand-dark/80 backdrop-blur-2xl px-6 py-24"
                dir={dir}
              >
                  <button onClick={() => setMobileMenuOpen(false)} className="absolute top-10 right-10 w-16 h-16 flex items-center justify-center rounded-[32px] bg-white/5 border border-white/10 text-white">
                      <X size={32}/>
                  </button>
                       ...
                       {navLinks.map((link, index) => (
                          ...
                          >
                              <Link 
                                ...
                                className="text-4xl font-black text-white hover:opacity-100 transition-opacity tracking-tighter"
